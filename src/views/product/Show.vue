...
export default {
  ...
  mounted() {
    ...
    if (localStorage.getItem('proID') != null) {
      ...
      axios
        .get(`http://167.172.71.33:31003/api/product/${localStorage.getItem('proID')}`)
        .then((response) => {
  ...
  methods: {
    ...
    delPro(){
  // 发起DELETE请求
    axios.delete(`http://167.172.71.33:31003/api/product/${localStorage.getItem('proID')}`, config)
  .then(response => {
    // 请求成功处理
  ...
  return dateStrings.map(dateString => {
    ...
    async submitNewPrice() {
      try {
        const url = `http://167.172.71.33:31003/api/product/${this.product.id}/submit-price?newPrice=${this.newPrice}`;
    ...
    //update product
      ...
      try {
        // 发送 POST 请求
        const response = await axios.put('http://167.172.71.33:31003/api/product/', formData, config);
